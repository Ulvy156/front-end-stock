<template>
  <article >
    <div class="flex items-center gap-x-2 mb-5">
      ...
      <commonHeader :title="$t('customers.details.created_by')" />
    </div>
    <section class="flex items-center gap-5">
      <imageForm class-name="col-span-3 w-[10%] h-[10%] rounded-full" :src="created_by?.img_url ?? ''" />
      <div class="grid grid-cols-3 gap-3">
        <commonContent> {{ $t('user.name') }}: {{ created_by?.name }} </commonContent>
        <div class="flex items-center gap-x-2">
          <commonContent>
            {{ $t('user.phone_number') }}: {{ formatPhoneDisplay(created_by?.phone ?? '') }}
          </commonContent>
          <iconCopy @click="copyToClipboard(formatPhoneDisplay(created_by?.phone ?? ''))" />
        </div>
        <commonContent>
          {{ $t('user.role') }}:
          <tagForm :title="created_by?.role" effect="dark" type="success"/>
        </commonContent>
        <commonContent>
          {{ $t('customers.details.created_at') }}: {{ toKhmerDateTime(created_by?.createdAt) }}
        </commonContent>
        <commonContent>
          {{ $t('user.email') }}: {{ created_by?.email }}
        </commonContent>
      </div>
    </section>
  </article>
